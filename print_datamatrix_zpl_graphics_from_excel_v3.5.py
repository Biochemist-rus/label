...
def resolve_resource_file(path_value: str) -> Path:
    external = Path(path_value)
    if external.exists():
        return external
    bundled = resource_path(path_value)
    if bundled.exists():
        return bundled
    return external
...
def get_worksheet(excel_file: str, sheet_name: Union[int, str]):
    ...
    if isinstance(sheet_name, int):
        ...
        return wb[sheet_names[sheet_name]]
...
def build_payload(cell_value) -> bytes:
    ...
    text = text.replace("_x001D_", "\x1d").replace("_x001d_", "\x1d")
...
def extract_ai_21(payload: bytes, max_len: int = 13) -> str:
    ...
    value = rest[:gs_pos] if gs_pos != -1 else rest
    value = value[:max_len].strip()
    return f"(21) {value}" if value else ""


def load_font(font_path: str, font_size: int) -> ImageFont.FreeTypeFont:
    path = resolve_resource_file(font_path)
    if not path.exists():
...
def load_and_prepare_logo(logo_path: str, logo_height_px: int) -> Image.Image:
    path = resolve_resource_file(logo_path)
    if not path.exists():
        raise FileNotFoundError(f"EAC logo file not found: {logo_path}")
    ...
    logo = logo.resize((max(1, int(round(w * scale))), max(1, int(round(h * scale)))), Image.LANCZOS)

    bg = Image.new("RGBA", logo.size, (255, 255, 255, 255))
    bg.alpha_composite(logo)
    logo = bg.convert("L")
    return logo.point(lambda p: 0 if p < 200 else 255, mode="1")
...
def build_text_lines(
    ...
    gtin_lines = wrap_text_by_pixels(normalize_text(gtin_text), gtin_font, max(40, text_width_px - 12), 1, draw)
    name_lines = wrap_text_by_pixels(normalize_text(name_text), name_font, max(40, text_width_px - 12), name_max_lines, draw)
    ai21_lines = wrap_text_by_pixels(normalize_text(ai21_text), ai21_font, max(40, text_width_px - 12), 1, draw)
...
def render_text_block_to_image(
    ...
    img = Image.new("L", (width_px, padding * 2 + text_height), 255)
    ...
    for y in range(height):
        ...
        for x in range(width):
            ...
            if bit_count == 8:
                ...
                bit_count = 0
        if bit_count > 0:
            row_bytes.append(current_byte << (8 - bit_count))
        rows_hex.append("".join(f"{b:02X}" for b in row_bytes))

    data = "".join(rows_hex)
    return f"^GFA,{total_bytes},{total_bytes},{bytes_per_row},{data}".encode("ascii")
...
def print_debug_info(
    ...
    print(f"[DEBUG] Version: {APP_VERSION}")
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--font-path", default=r"fonts\arial.ttf", help="Path to TTF font")
