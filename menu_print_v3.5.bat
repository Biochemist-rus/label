...
set "VERSION=3.5"
set "EXE=dist\print_datamatrix_zpl_graphics_from_excel_v3.5.exe"
set "INI=menu_print_v3.5.ini"
set "LOGDIR=logs"
...
if not exist "%EXE%" (
  ...
  echo.
  echo Build it first with: build_exe_v3.5.bat
  echo Сначала соберите его через: build_exe_v3.5.bat
  pause
  exit /b 1
)
...
echo   menu_print_v3.5.bat
echo   menu_print_v3.5.ini
echo   data.xlsx
echo   eac.png
echo   fonts\arial.ttf
echo   dist\print_datamatrix_zpl_graphics_from_excel_v3.5.exe
...
:save_ini
> "%INI%" echo ; LABEL PRINTING TOOL v3.5 CONFIGURATION / КОНФИГУРАЦИЯ ПЕЧАТИ ЭТИКЕТОК v3.5
